...
public class PhysicsCalculations {
	...
	public static final double time = 60;  // one minute in seconds
	public static final double pixToM = 1900000;
	
	// using meters, seconds, kilograms
	
	
	public static double forceGravity(double mass1, double mass2, double obj1X, double obj2X, double obj1Y, double obj2Y) {
		// F = G(m1*m2)/r^2
		return G*(mass1*mass2)/(Math.pow((obj1X-obj2X)*pixToM, 2)+Math.pow((obj1Y-obj2Y)*pixToM, 2)); 
	}
	
	public static double positionX(double objX, double objV) throws IOException {
		// xf = xi + vi*dt
		// or use xf = xi + vi*dt + 0.5*a*dt^2
		FileOutputStream out = new FileOutputStream("v_ops_test.txt");
	...
	public static double velocityX(double obj1V, double obj1Force, double obj1M) {
		// vf = vi + a(dt)
		// F = m*a
		// a = F/m
		// vf = vi + F/m * dt
